...
int bichitosX = 4; //Variables para la matriz
int bichitosY = 8;
...
void generate() {
  ...
  for (int i=0; i < bichitos.length; i++) { //Itineramos por los 100 elementos del array
    for (int j=bichitosGapX; j < width; j=j+bichitosGapX) { //matriz horizontal
      for (int k=bichitosGapY; k < height; k=k+bichitosGapY) { //matriz vertical
